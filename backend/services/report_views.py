"""
backend/services/report_views.py
===================================
...
def generate_report(request):
    ...
    Body: {
        ...
        "period": {
            ...
            "end_date": "2025-12-31",
            "report_type": "monthly" | "quarterly" | "annual" | "ytd"
        }
    }
    """
    try:
    ...
    # Ajouter la période aux paramètres
    params["period"] = period
    params["current_date"] = period.get("start_date", timezone.now().isoformat())
    params["analysis_period"] = period.get("report_type", "monthly")
    params["start_date"] = period.get("start_date")
    params["end_date"] = period.get("end_date")

    if report_type not in ("market", "investment"):
        return JsonResponse(
            {"error": "type must be 'market' or 'investment'"},
            status=400
        )
...
def export_report_pdf(request, pk: int):
    ...
    if logo_path.exists():
        img = Image(str(logo_path), width=1.5*inch, height=0.4*inch)
        img.hAlign = 'LEFT'
        story.append(img)
