...
class VerdarScraper(BaseScraper):
    ...
    def parse_listing(self, html_content: str, url: str, listing_id: str, transaction_type: str) -> PropertyListing:
        ...
        price = 0.0
        # Heuristic for price
        price_tags = soup.find_all(text=re.compile(r'DT'))
        for pt in price_tags:
            clean = re.sub(r'[^\d]', '', pt)
            if len(clean) > 3: # Avoid small numbers
                try:
                    price = float(clean)
                    break
                except:
        ...
        return PropertyListing(
            ...
            location={"city": "Tunis", "governorate": "Tunis"}, # Placeholder
