...
class DarcomScraper(BaseScraper):
    """
    Scraper for Darcom Tunisia (darcomtunisia.com).
    """
    def __init__(self):
        super().__init__(source_name="darcom", base_url="https://www.darcomtunisia.com")
        self.categories = [
            {"url": f"{self.base_url}/vente", "type": "Sale"},
            {"url": f"{self.base_url}/location", "type": "Rent"}
        ]
    ...
    def parse_listing(self, html_content: str, url: str, listing_id: str, transaction_type: str) -> PropertyListing:
        ...
        price = 0.0
        price_text = ""
        # Look for price in specific divs or meta tags usually found in this CMS
        price_tag = soup.find("div", class_="price") or soup.find(text=re.compile(r'\d+.*DT'))
        if price_tag:
        ...
        # Location extraction
        location_text = ""
        loc_tag = soup.find("div", class_="location") # Heuristic
        if loc_tag:
            location_text = loc_tag.get_text(strip=True)

        description = ""
        desc_tag = soup.find("div", class_="description") or soup.find("div", id="description")
        if desc_tag:
        ...
        return PropertyListing(
            ...
            location={"city": location_text, "governorate": "Tunis", "address": location_text}, # Fallback
