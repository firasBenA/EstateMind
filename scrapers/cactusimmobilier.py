...
class CactusImmobilierScraper(BaseScraper):
    def __init__(self):
        super().__init__(source_name="cactusimmobilier", base_url="https://www.cactusimmobilier.com")
        self.start_urls = [self.base_url]

    def fetch_listings(self) -> Generator[PropertyListing, None, None]:
        seen = set()
        resp = self._get_request(self.base_url)
        if not resp:
            return
        ...
        for a in soup.find_all("a", href=True):
            ...
            low = href.lower()
            if self.base_url in href and any(token in low for token in ["vente", "vendre", "location", "louer", "annonce", "biens", "catalogue", "properties"]):
                cat_candidates.add(href)
        cats = list(cat_candidates)[:8]
        if not cats:
            cats = [self.base_url]
        for cat in cats:
            rcat = self._get_request(cat)
            if not rcat and not cat.endswith("/"):
                rcat = self._get_request(cat + "/")
            if not rcat:
                continue
            csoup = BeautifulSoup(rcat.text, "html.parser")
            ...
            for a in links:
                ...
                if not href.startswith("http"):
                    href = f"{self.base_url.rstrip('/')}/{href.lstrip('/')}"
                low = href.lower()
                if self.base_url in href and any(token in low for token in ["annonce", "bien", "details", "property", "immobilier"]):
