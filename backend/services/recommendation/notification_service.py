...
from services.models import UserNotification, UserBehaviorLog, Listing
...
class NotificationService:
    ...
    def send_new_listing_notification(user_id, listing_data):
        ...
        pusher_client.trigger(
            f'private-user-{user_id}',   # ✅ private- prefix
            'new-listing',
            {
                'id': notification.id,
    ...
    def send_price_drop_notification(user_id, listing_id, old_price, new_price):
        notification = UserNotification.objects.create(
            ...
            message=f"Price dropped from {old_price:,.0f} TND to {new_price:,.0f} TND",
